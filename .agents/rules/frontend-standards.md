...
- **Discourage `any`**: The use of `any` is strongly discouraged. Define explicit interfaces or types for all data.
  - _Exceptions_: Allowed for third-party libraries without types, complex migration steps, or quick debugging helpers. Document exceptions with a comment.
- **Model IDs**: Use the `EntityId` type for all identifiers. `EntityId` is a primitive type representable as `string | number`.
  - _Example_: `type EntityId = string | number;` (Refer to `src/types/index.ts` for the canonical definition).
...
- **Global Context**: Prefer `globalThis` over `window` for accessing global properties (e.g., `globalThis.innerWidth`, `globalThis.scrollTo`). In tests, use `window` directly for browser tests or testing library utilities for `fireEvent`. Ensure your TypeScript configuration (`lib: ["dom"]`) is set so `globalThis` is already compatible with the global scope.
